...
import { parseXlsxQuestionnaire } from "./xlsx.js";

export type SourceFormat = "csv" | "sig_lite" | "xlsx";
...
export function ingest(format: SourceFormat, content: string | Buffer): QuestionnaireItem[] {
  if (format === "xlsx") return parseXlsxQuestionnaire(Buffer.isBuffer(content) ? content : Buffer.from(content));
  const text = Buffer.isBuffer(content) ? content.toString("utf8") : content;
  return format === "sig_lite" ? parseSigLite(text) : parseCsvQuestionnaire(text);
}
