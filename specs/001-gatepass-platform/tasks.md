...
- [~] T026 [US1] LLM gateway client: Gatepass-owned accounts, zero-retention headers, per-org disable flag with static-fallback signal in packages/semantic/src/gateway/ — DONE: LlmGateway with zero-retention tagging, per-org disable, offline fallback, confidence parsing (built+tested); live Anthropic transport deferred
